...
if(process.env.NETLIFY==='true'||process.env.PREPARE_COVERS==='1'){
  ...
  const queue=Object.entries(seeds);
  async function prepare([id,seed]){
    try{
    ...
    if(seed.pdfSha256&&hash!==seed.pdfSha256){console.warn(`Official PDF changed for ${id}; photo awaits review`);return;}
    ...
    if(!photo){console.warn(`No verified cover for ${id}`);return;}
  ...
  await Promise.all(Array.from({length:3},async()=>{while(queue.length)await prepare(queue.shift());}));
