...
async function fetchHtml(source){
  try{const r=await f(source,{headers:{...hdr,accept:'text/html,application/xhtml+xml,*/*;q=0.8'}},15000);if(r.ok)return {html:await r.text(),via:'direct',status:r.status,finalUrl:r.url||source};}catch{}
  try{const r=await f('https://r.jina.ai/'+source,{headers:{'user-agent':UA,accept:'text/plain'}},18000);if(r.ok)return {html:htmlFromMarkdown(await r.text()),via:'jina',status:r.status,finalUrl:source};}catch{}
...
function imageCandidates(html,base){
  const out=[];const push=(url,score,why)=>{const abs=resolveUrl(url,base);if(abs)out.push({url:abs,score,why});};
  for(const re of [/<meta[^>]+property=["']og:image(?::secure_url)?["'][^>]+content=["']([^"']+)["'][^>]*>/ig,/<meta[^>]+content=["']([^"']+)["'][^>]+property=["']og:image(?::secure_url)?["'][^>]*>/ig]){let m;while((m=re.exec(html)))push(m[1],120,'meta');}
  let im;const imgRe=/<img\b[^>]*>/ig;while((im=imgRe.exec(html))){const tag=im[0],alt=(tag.match(/alt=["']([^"']*)["']/i)||[])[1]||'',cls=(tag.match(/class=["']([^"']*)["']/i)||[])[1]||'';for(const a of ['data-original','data-src','data-lazy-src','src']){const m=tag.match(new RegExp(`${a}=["']([^"']+)["']`,'i'));if(m)push(m[1],55+(/物件|写真|外観|建物|photo|gallery/i.test(alt+' '+cls)?50:0),a);}}
  const seen=new Set();return out.filter(x=>{if(seen.has(x.url))return false;seen.add(x.url);const u=x.url.toLowerCase();if(/logo|icon|favicon|sprite|avatar|banner|loading|noimage|placeholder|google|map/.test(u))x.score-=120;if(/photo|picture|image|img|bukken|property|media/.test(u))x.score+=25;return x.score>0;}).sort((a,b)=>b.score-a.score);
}
...
async function fetchResource(item,referer){
  ...
  if(ct.includes('pdf')||head.startsWith('%PDF'))return {kind:'pdf',contentType:'application/pdf',bytes:buf,finalUrl:r.url||item.url};
...
async function bestImageFromHtml(html,base){
  for(const c of imageCandidates(html,base).slice(0,30)){
    try{const r=await f(c.url,{headers:{'user-agent':UA,accept:'image/avif,image/webp,image/*,*/*;q=0.8',referer:base}},12000);const ct=(r.headers.get('content-type')||'').toLowerCase();if(!r.ok||!ct.startsWith('image/'))continue;const buf=await r.arrayBuffer();if(buf.byteLength<12000||buf.byteLength>15_000_000)continue;return {body:buf,contentType:ct,method:'bit-page-image',source:c.url,page:null};}catch{}
  }
  return null;
}
...
export default async(req)=>{
  ...
  const discovered=documentCandidates(page.html,source).slice(0,10);
  ...
  await docsStore.set(`${id}/source.html`,page.html,{metadata:{contentType:'text/html; charset=utf-8',sourceUrl:source,fetchedAt:new Date().toISOString(),via:page.via}});
  ...
  while(queue.length&&saved.length<8){
    ...
    try{
      ...
      if(res.kind==='pdf'){
        try{const p=await extractLargestPhotoFromPdf(res.bytes,{maxPages:64});if(p&&(!bestPhoto||item.type==='現況調査報告書'||item.type==='三點件一括'))bestPhoto={...p,from:item.type||'document',source:item.url};}catch{}
        ...
        if(!bestPhoto){const p=await bestImageFromHtml(res.html,item.url);if(p)bestPhoto={...p,from:item.type||'法院頁'};}
      }
  ...
  if(!bestPhoto){try{bestPhoto=await bestImageFromHtml(page.html,source);}catch{}}
  let mainImage=null;if(bestPhoto?.body){const body=bestPhoto.body instanceof ArrayBuffer?bestPhoto.body:bestPhoto.body.buffer.slice(bestPhoto.body.byteOffset,bestPhoto.body.byteOffset+bestPhoto.body.byteLength);await media.set(`${id}/cover`,body,{metadata:{contentType:bestPhoto.contentType||'image/jpeg',method:bestPhoto.method||'three-doc',source:bestPhoto.source||null,pdfType:bestPhoto.from||null,pdfPage:bestPhoto.page||null,fetchedAt:new Date().toISOString()}});mainImage={storageKey:`${id}/cover`,contentType:bestPhoto.contentType||'image/jpeg',method:bestPhoto.method||'three-doc',pdfType:bestPhoto.from||null,pdfPage:bestPhoto.page||null};}

  const record={caseId:id,sourceUrl:source,sourceSite:new URL(source).hostname,ingestedAt:new Date().toISOString(),pageVia:page.via,courtId:ids.courtId,saleUnitId:ids.saleUnitId,bitUrl:primaryBit?.url||null,hints:{title:input.title||null,prefecture:input.prefecture||null,city:input.city||null,court:input.court||null,price:input.price||null,type:input.type||null,area:input.area||null,bid:input.bid||null},documents:saved,mainImage};
  await index.setJSON(`cases/${id}`,record);
  const successful=saved.filter(x=>x.saved).length;
  ...
  return Response.json({ok:true,...record,storage:'netlify-blobs',savedCount:successful});
};
