...
export default async()=>{
 const store=discoveryStore(),now=new Date(),started=Date.now();const current=await store.get('current',{type:'json'})||[];
 const targets=current.filter(x=>x.detailState!=='ready'&&(!x.nextDetailCheckAt||Date.parse(x.nextDetailCheckAt)<=now.getTime())).sort((a,b)=>String(a.nextDetailCheckAt||'').localeCompare(String(b.nextDetailCheckAt||''))).slice(0,8);
 for(const item of targets){
  if(Date.now()-started>660000)break;
  let updated;
  try{
  ...
  }catch(e){updated={...(updated||item),detailState:'retry',detailError:e.message,nextDetailCheckAt:new Date(Date.now()+45*60000).toISOString()};}
